...
fn test_circuit_breaker_prevents_runaway_failures() {
    ...
    for i in 0..3 {
        assert!(
            !breaker.should_skip(domain),
            "Should not skip before threshold at iteration {}",
            i
        );
    ...
    assert!(
        breaker.should_skip(domain),
        "Should skip after reaching threshold"
    );
...
fn test_config_prevents_unbounded_growth() {
    ...
    assert!(
        ...
        "Queue limit should be reasonable"
    );
    assert!(config.max_memory_mb > 0, "Memory limit must be set");
    assert!(
        config.max_memory_mb <= 10000,
        "Memory limit should be reasonable (< 10GB)"
    );
}
