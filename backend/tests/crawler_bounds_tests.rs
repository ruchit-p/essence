...
fn test_crawler_config_defaults() {
    ...
    assert_eq!(
        ...
        "Default max_queue_size should be 1000"
    );
    assert_eq!(
        config.max_memory_mb, 500,
        "Default max_memory_mb should be 500"
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        "Default circuit_breaker_threshold should be 3"
    );
    assert_eq!(
        config.backpressure_threshold, 80,
        "Default backpressure_threshold should be 80"
    );
    assert!(
...
fn test_circuit_breaker_threshold() {
    ...
    assert!(
        !breaker.should_skip("example.com"),
    ...
    assert!(
        breaker.should_skip("example.com"),
        "Should skip at 3 failures (threshold reached)"
    );
    ...
    assert!(
        breaker.should_skip("example.com"),
        "Should still skip at 4 failures"
    );
}
...
fn test_circuit_breaker_multiple_domains() {
    ...
    // Check skip status
    assert!(
        breaker.should_skip("domain1.com"),
        "domain1.com should be skipped"
    );
    assert!(
        breaker.should_skip("domain2.com"),
        "domain2.com should be skipped"
    );
    assert!(
        !breaker.should_skip("domain3.com"),
        "domain3.com should not be skipped"
    );

    // Check total failures
    assert_eq!(
        breaker.get_total_failures(),
        2,
        "Should have 2 domains in failure state"
    );
    ...
    assert!(
        !breaker.should_skip("domain1.com"),
        "domain1.com should not be skipped after reset"
    );
    assert_eq!(
        breaker.get_total_failures(),
        1,
        "Should have 1 domain in failure state"
    );
}
...
fn test_memory_monitor_disabled() {
    ...
    assert!(
        monitor.check_memory_limit().is_ok(),
...
fn test_memory_monitor_enabled() {
    ...
    assert!(
        percentage > 0.0,
        "Should return non-zero percentage when enabled"
    );
}
...
fn test_queue_size_calculations() {
    ...
    assert_eq!(
        backpressure_limit, 800,
        "Backpressure limit should be 80% of max_queue_size"
    );

    // Verify max_queue_size is reasonable
    assert!(
        config.max_queue_size >= 100,
        "max_queue_size should be at least 100"
    );
    assert!(
        config.max_queue_size <= 10000,
        "max_queue_size should not exceed 10000"
    );
}
...
fn test_config_serialization() {
    ...
    assert!(
        json.contains("maxQueueSize"),
        "JSON should contain maxQueueSize in camelCase"
    );
...
fn test_config_custom_values() {
    ...
    let config: CrawlerConfig =
        serde_json::from_str(json).expect("Should deserialize custom config");
